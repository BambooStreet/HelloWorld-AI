...
def generate_ai_response(conversation_history,query,db):
    ...
    template_text = """
    You are a competent AI assistant. Please refer to the following related articles to help you generate appropriate responses to conversations. Please answer in Indonesian.\n\nRelated documentation: 
    {context}
...
def voice():
    try :
        ...
        gather = Gather(input='speech', language='id-ID', timeout=3, action='/process_speech')
        gather.say("Halo, ini Hello World, konselor Anda untuk pekerja asing!! Bagaimana saya bisa membantu Anda?"
                   , language="id-ID")
...
def request_input():
    response = VoiceResponse()
    response.say("Beri tahu kami jika Anda memiliki pertanyaan tambahan.", language="id-ID")
    gather = Gather(input='speech', language='id-ID', timeout=3, action='/process_speech')
    response.append(gather)
    return str(response)
...
def process_speech():
    ...
    if user_speech:
        ...
        #GPT가 생성한 음성 반환
        response.say(ai_response, language="id-ID")
    ...
    #음성 반환이 안되면,
        ...
        response.say("Kami tidak menerima input suara Anda, silakan coba lagi.", language="id-ID")
