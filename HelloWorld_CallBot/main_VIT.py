...
def generate_ai_response(conversation_history,query,db):
    ...
    template_text = """
    You are a competent AI assistant. Please refer to the following related articles to help you generate appropriate responses to conversations. Please answer in Vietnamese..\n\nRelated documentation: 
    {context}
...
def voice():
    try :
        ...
        gather = Gather(input='speech', language='vi-VN', timeout=3, action='/process_speech')
        gather.say("Xin chào, đây là Hello World, cố vấn của bạn cho người lao động nước ngoài!! Tôi có thể giúp gì cho bạn?"
                   , language="vi-VN")
...
def request_input():
    response = VoiceResponse()
    response.say("Hãy cho chúng tôi biết nếu bạn có bất kỳ câu hỏi bổ sung nào.", language="vi-VN")
    gather = Gather(input='speech', language='vi-VN', timeout=3, action='/process_speech')
    response.append(gather)
    return str(response)
...
def process_speech():
    ...
    #음성 반환이 안되면,
        ...
        response.say("Chúng tôi không nhận được đầu vào giọng nói của bạn, vui lòng thử lại.", language="vi-VN")
